...
void merge_couplestrees(int platenumber){
 ...
 //histo file
 outputfile = new TFile(Form("/eos/user/f/falicant/handson_FEDRA/b%0*i/p%0*i/%i.%i.0.0_merged.cp.root", 6, brickID, 3, platenumber, brickID, platenumber),"RECREATE"); 
 ...
 TString prepath(Form("/eos/user/f/falicant/handson_FEDRA/b%0*i/p%0*i/%i.%i.",6, brickID, 3 , platenumber, brickID,platenumber));
